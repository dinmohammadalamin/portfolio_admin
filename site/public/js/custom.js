...
function SendContact(contact_name, contact_mobile, contact_email, contact_msg) {
    if (contact_name.length == 0) {
        $("#contactSendBtnId").html("Please write your name");
        setTimeout(function () {
            $("#contactSendBtnId").html("Send");
        }, 2000);
    } else if (contact_mobile.length == 0) {
        $("#contactSendBtnId").html("Please Enter your mobile number");
        setTimeout(function () {
            $("#contactSendBtnId").html("Send");
        }, 2000);
    } else if (contact_email.length == 0) {
        $("#contactSendBtnId").html("Please Write your email");
        setTimeout(function () {
            $("#contactSendBtnId").html("Send");
        }, 2000);
            ...
            .then(function (response) {
                if (response.status == 200) {
                    if (response.data == 1) {
                        ...
                        $("#contactSendBtnId").html("Failed Message ");
                        setTimeout(function () {
                            $("#contactSendBtnId").html("Send");
                        }, 3000);
                    }
